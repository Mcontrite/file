...
func GetFileMetaList(limit int) ([]TableFile, error) {
	stmt, err := mydb.DBConn().Prepare("select file_sha1,file_addr,file_name,file_size from tbl_file where status=1 limit ?")
	if err != nil {
		fmt.Println(err.Error())
		return nil, err
	}
	...
	fmt.Println("Executing GetFileMetaList, len(tfiles)= ", len(tfiles))
	return tfiles, nil
}
...
func OnFileUploadFinished(filehash, filename string, filesize int64, fileaddr string) bool {
	...
	if rf, err := ret.RowsAffected(); nil == err {
		...
		fmt.Println("Execute OnFileUploadFinished Finished OK...")
		return true
	}
	fmt.Println("Execute OnFileUploadFinished Finished False...")
	return false
}
